...
namespace mcc {
  static const uint32_t kSeed = 0;

#define BIG_CONSTANT(x) (x##LLU)

  static inline uint32_t
  rotl32(const uint32_t x, const uint8_t r) {
    return (x << r) | (x >> (32 - r));
  }

  static inline uint64_t
  rotl64(const uint64_t x, const uint8_t r) {
    return (x << r) | (x >> (64 - r));
  }

  static inline uint32_t
  fmix32(const uint32_t hash) {
    auto h = hash;
    h ^= h >> 16;
    h *= 0x85ebca6b;
    h ^= h >> 13;
  ...
  uint128 murmur3_128(const uint8_t* bytes, const uword num_bytes) {
    ...
    const int nblocks = num_bytes / 16;
    int i;
    uint64_t h1 = kSeed;
    uint64_t h2 = kSeed;
