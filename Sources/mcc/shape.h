...
namespace mcc {
  namespace shape {
    ...
    template<typename Vertex, typename Index>
    void NewTexturedQuad(std::vector<Vertex>& vertices,
                         std::vector<Index>& indices,
                         const glm::vec2& topLeft,
                         const glm::vec2& bottomLeft,
                         const glm::vec2& bottomRight,
...
#define HALF_HEIGHT (HEIGHT / 2)
    ...
    template<typename Vertex, typename Index>
    void NewTexturedCenteredRect(std::vector<Vertex>& vertices,
                                 std::vector<Index>& indices,
                                 const glm::vec2& pos,
                                 const glm::vec2& size,
                                 const Color& color = kBlack) {
      ...
      const auto bottomRight = glm::vec2(X + HALF_WIDTH, Y - HALF_HEIGHT);
      const auto topRight = glm::vec2(X + HALF_WIDTH, Y + HALF_HEIGHT);
      return NewTexturedQuad<Vertex, Index>(vertices, indices, topLeft, bottomLeft, bottomRight, topRight, color);
    }
  }
}
